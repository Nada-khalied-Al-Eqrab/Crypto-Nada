# شامل جميع خوارزميات التشفير المذكورة
import string
import random
# import numpy as np
1
...
def modinv(a, m):
    for i in range(1, m):
        if (a * i) % m == 1:
            return i
    raise ValueError("لا يوجد معكوس للمفتاح")
...
def rotor_encrypt(text, rotor):
    ...
    for i, char in enumerate(text):
        result += chr((ord(char) + rotor[i % len(rotor)]) % 256)
    return result
...
def enigma_encrypt(text, rotor1, rotor2):
    result = ''
    for i, char in enumerate(text):
        temp = (ord(char) + rotor1[i % len(rotor1)]) % 256
        result += chr((temp + rotor2[i % len(rotor2)]) % 256)
    return result

def enigma_decrypt(text, rotor1, rotor2):
    result = ''
    for i, char in enumerate(text):
        temp = (ord(char) - rotor2[i % len(rotor2)]) % 256
        result += chr((temp - rotor1[i % len(rotor1)]) % 256)
    return result
...
# -------------------- 15. القائمة التفاعلية --------------------
def main():
    print("\n برنامج كربتو ندى  لخوارزميات التشفير\n")
    print("اختر العملية:")
    print("1. تشفير")
    print("2. فك التشفير")
    mode = input("أدخل رقم العملية: ")
    ...
    if cipher_choice == 1:
        ...
        key = input("أدخل الكلمة المفتاحية: ")
        ...
        print("مثال على مصفوفة المفتاح: [[3,3],[2,5]]")
        matrix = eval(input("أدخل مصفوفة المفتاح 2x2 أو 3x3 كقائمة بايثون: "))
        result = hill_encrypt(text, np.array(matrix)) if mode == '1' else hill_decrypt(text, np.array(matrix))
    elif cipher_choice == 9:
        key = input("أدخل مفتاح بطول النص تمامًا: ")
        result = otp_encrypt(text, key) if mode == '1' else otp_decrypt(text, key)
    elif cipher_choice == 10:
        rotor = list(map(int, input("أدخل سلسلة أرقام التحريك مفصولة بمسافة: ").split()))
        result = rotor_encrypt(text, rotor) if mode == '1' else rotor_decrypt(text, rotor)
    elif cipher_choice == 11:
        rotor1 = list(map(int, input("أدخل الدوار الأول: ").split()))
        rotor2 = list(map(int, input("أدخل الدوار الثاني: ").split()))
        ...
        key = input("أدخل الكلمة المفتاحية (مثلاً ZEBRAS): ")
